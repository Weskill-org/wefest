...
import { 
  Share2, Key, Globe, Plus, Loader2, Trash2, CheckCircle2, 
  ExternalLink, Copy, Zap, Info, ShieldCheck 
} from "lucide-react";
  ...
  return (
    <div className="space-y-10">
      <div>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage API access and webhooks for institutional partners.</p>
          </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="glass rounded-2xl p-6">
          ...
          <div className="space-y-4">
            {apiKeys?.length === 0 ? (
              <p className="text-sm text-muted-foreground italic text-center py-4">No active API keys found.</p>
            ) : (
              apiKeys?.map((key: any) => (
                <div key={key.id} className="flex items-center justify-between p-4 rounded-xl bg-muted/30 border border-border/40">
                  <div>
                    <div className="font-bold text-sm">{key.college?.name}</div>
                    <div className="flex items-center gap-2 mt-1">
                      <code className="text-[10px] text-muted-foreground bg-muted px-2 py-0.5 rounded">wf_••••••••••••</code>
                      <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => toast.success("Key copied to clipboard")}>
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <div className="space-y-4">
            {webhooks?.length === 0 ? (
              <p className="text-sm text-muted-foreground italic text-center py-4">No configured webhooks found.</p>
            ) : (
              webhooks?.map((wh: any) => (
                <div key={wh.id} className="p-4 rounded-xl bg-muted/30 border border-border/40">
                  ...
                  <div className="text-[10px] text-muted-foreground font-mono truncate">{wh.url}</div>
                  <div className="mt-3 flex gap-2">
                    {wh.events.map((e: string) => (
                      <Badge key={e} variant="secondary" className="text-[9px] uppercase">{e}</Badge>
                    ))}
                  </div>
                </div>
              ))
        ...
        <div className="flex flex-col md:flex-row items-center gap-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-2 max-w-2xl">
              Enable your university's internal systems to sync with WeFest. We provide native support for major College ERPs (SAP Campus, Oracle, etc.) and custom webhooks for real-time attendance and event data.
            </p>
            <div className="mt-6 flex flex-wrap gap-4">
              <Button size="sm" variant="outline" className="bg-white/5 border-white/10">View API Docs <ExternalLink className="ml-2 h-3 w-3" /></Button>
              <Button size="sm" variant="outline" className="bg-white/5 border-white/10">Webhook Secrets</Button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Feature icon={ShieldCheck} title="Verified Payloads" desc="All webhooks are signed with SHA-256 for maximum security." />
        <Feature icon={Zap} title="Instant Sync" desc="Real-time ticket validation syncing with college database." />
        <Feature icon={Info} title="Compliance" desc="Fully compliant with UGC and state-level data guidelines." />
      </div>
    </div>
  );
}
