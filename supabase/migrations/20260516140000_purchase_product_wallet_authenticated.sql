-- Campus store purchase callable with the user's JWT (no app-server service role required)

DROP FUNCTION IF EXISTS public.purchase_product_with_wallet(uuid, uuid, int, text);
...
REVOKE ALL ON FUNCTION public.purchase_product_with_wallet(uuid, int, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.purchase_product_with_wallet(uuid, int, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.purchase_product_with_wallet(uuid, int, text) TO service_role;
